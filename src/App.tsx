...
function App() {
  ...
  const dockItems = [
    ...
    // Only shown once real recommendations exist — the section itself renders nothing
    // until then (see TestimonialsSection.tsx), so this item would otherwise dead-link.
  ...
  return (
    <div className="bg-transparent min-h-screen relative overflow-x-hidden selection:bg-primary/30 selection:text-primary-foreground">
      ...
      <ReactLenis
        root
        options={{ smoothWheel: !prefersReducedMotion, duration: prefersReducedMotion ? 0 : 1.2 }}
      >
        <Header />
